...
class Student:
    def __init__(self,root):
        ...
        img_btn3 = img_btn3.resize((105, 35), PIL.Image.BILINEAR)
        ...
        # Place the "Export to Excel" button next to the "View All" button
        ...
        btn_export_excel = Button(search_frame, text="Xuất Excel", font=("times new roman", 11, "bold"),
                                  command=self.export_to_excel, bd=0, bg="white", cursor='hand2',
                                  activebackground='white',
                                  width=105, image=self.photobtn3, fg="white", compound="center")
        btn_export_excel.grid(row=0, column=3, padx=4)
        ...
        #các trường dữ liệu
        self.student_table=ttk.Treeview(table_frame,column=("id","name","div","roll","gender","dob","email","phone","address","photo"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.student_table.heading("div", text="Nghành học")
    ...
    #============function decration===============
    def student_image(self):#Xem ảnh của
        if self.var_std_name.get()=="" or self.var_std_id.get()=="":#Nếu txt tên hs hoặc id trống thì báo lỗi
            messagebox.showerror("Error","Vui lòng nhập đầy đủ thông tin",parent=self.root)
        else:
            student_id(self.var_std_id.get())#truyền id  vào form thông tin ảnh
    ...
    def add_data(self):#Thêm dữ liệu
        ...
        if  self.var_std_name.get()=="" or self.var_std_id.get()=="" or self.var_div.get()=="":
            ...
            try:
                ...
                messagebox.showinfo("Thành công","Thêm thông tin người dùng thành công",parent=self.root)#thêm thành công
            except Exception as es:#Nếu có lỗi -> in ra màn hình
                messagebox.showerror("Error",f"Due To:{str(es)}",parent=self.root)


    #=======================fetch-data========================
    def fetch_data(self):#Lấy tất cả thông tin
        conn=mysql.connector.connect(host=self.db.host, user=self.db.user, password=self.db.password, database=self.db.database, port=self.db.port)

        my_cursor = conn.cursor()
        #câu lệnh sql truy vấn tất cả thông tin
        my_cursor.execute("Select student_id,name,class,roll,gender,dob,email,phone,address,photosample from student")
    ...
    def update_data(self):#Cập nhật thông tin
        ...
        if  self.var_std_name.get()=="" or self.var_std_id.get()=="":
            ...
            try:
                ...
                Update=messagebox.askyesno("Update","Bạn có muốn cập nhật thông tin người dùng này không?",parent=self.root)
                if Update>0:#nếu bấm yes
                    conn=mysql.connector.connect(host=self.db.host, user=self.db.user, password=self.db.password, database=self.db.database, port=self.db.port)
                    my_cursor = conn.cursor()
                    #câu lệnh sql update các thong tin
                    my_cursor.execute("update student set Name=%s,Class=%s,"
    ...
    #Delete Function
    def delete_data(self):#Xóa thông tin theo mã id
        if self.var_std_id.get()=="":
            messagebox.showerror("Lỗi","Không được bỏ trống ID",parent=self.root)
        else:
            try:
                delete=messagebox.askyesno("Xoá ","Bạn có muốn xóa user này?",parent=self.root)
                if delete>0:
                        conn = mysql.connector.connect(host=self.db.host, user=self.db.user, password=self.db.password, database=self.db.database, port=self.db.port)
                        my_cursor = conn.cursor()
                        sql="delete from student where Student_id=%s"#Câu lệnh xóa  theo id
                ...
                self.reset_data()
                conn.close()
                messagebox.showinfo("Xóa","Xóa user thành công",parent=self.root)
            except Exception as es:
                messagebox.showerror("Lỗi",f"Due To:{str(es)}",parent=self.root)
    ...
    def search_data(self):
        if self.var_search.get() == "":
            ...
            try:
                ...
                my_cursor = conn.cursor()
                # Câu lệnh tìm kiếm trả về các trường dữ liệu được chỉ định
                my_cursor.execute(
                    "SELECT Student_id, Name, Class, Roll, Gender, Dob, Email, Phone, Address, PhotoSample FROM student WHERE Student_id LIKE '%" + self.var_search.get() + "%' OR Name LIKE '%" + self.var_search.get() + "%'")
                data = my_cursor.fetchall()
                # Số lượng bản ghi tối đa muốn hiển thị
                max_records_to_display = 10  # Số lượng bản ghi muốn hiển thị
                if len(data) != 0:
                    self.student_table.delete(*self.student_table.get_children())
                    # Chỉ hiển thị một số bản ghi tối đa
                    for i in data[:max_records_to_display]:
                        self.student_table.insert("", END, values=i)
                    if len(data) > max_records_to_display:
                        messagebox.showinfo("Thông báo", f"Chỉ hiển thị {max_records_to_display} bản ghi đầu tiên",
                                            parent=self.root)
                    else:
    ...
    def generate_dataset(self):#Chụp ảnh
        regex = '^[a-z0-9]+[\._]?[a-z0-9]+[@]\w+[.]\w{2,3}$'  # check email
        if  self.var_std_name.get()=="" or self.var_std_id.get()=="":
            messagebox.showerror("Error","Vui lòng nhập đầy đủ thông tin",parent=self.root)
        elif (re.search(regex, self.var_email.get()) == None):
    ...
    def train_classifier(self):#Hàm train model nhận diện
        ...
        for image in path:#với từng ảnh trong thư mục
            ...
            id=int(os.path.split(image)[1].split('.')[1])#Lấy ra i từ tên của ảnh( ví dụ: user.1.19.jpg -> id =1)
    ...
    def search_Classdata(self):#Tìm thông tin lớp học
            if self.var_com_searchclass.get() == "" or self.var_searchclass.get() == "":
                ...
                try:
                    conn = mysql.connector.connect(host=self.db.host, user=self.db.user, password=self.db.password, database=self.db.database, port=self.db.port)
                    my_cursor = conn.cursor()  # "ID Điểm Danh", "Ngày", "ID"
    ...
    def export_to_excel(self):
        try:
            ...
            my_cursor = conn.cursor()
            my_cursor.execute(
                "SELECT Name, Class, Roll, Gender, DOB, Email, Phone, Address FROM student")  # Select specific columns from the student table
            data = my_cursor.fetchall()
            ...
            if len(data) > 0:
                ...
                # Add column headers
                ws.append(
                    ["Họ tên", "Nghành học", "CMND", "Giới tính", "Ngày sinh", "Email", "Số điện thoại", "Địa chỉ",
                     ])
                ...
                # Save the workbook directly to the specified directory
                file_path = "D:/Nghiên cứu khoa học-2024/face_recognizer/Thong_Ke/Dữ liệu người dùng có trong hệ thống.xlsx"
                wb.save(file_path)
                messagebox.showinfo("Success", f"Student data exported to {file_path} successfully.")
